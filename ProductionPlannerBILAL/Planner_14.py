...
def generate_timeline(df):
    ...
    Fixes included:
      - Scheduled washes run continuously every (Duration + Gap).
      - Intermediate wash is scheduled 24 hours AFTER the processing START
        that follows a wash (not 24 hours of processing).
      - Any wash (scheduled, additional, standalone intermediate) resets the
        24-hour clock so the next processing start becomes the origin.
    ...
    if first_wash_time and wash_duration > timedelta(0):
        st.write(f"🟡 FIRST WASH: Adding wash at {first_wash_time.strftime('%H:%M')}")
        tasks.append({
        ...
        st.write(f"🟡 FIRST WASH: Set reset_24h_on_next_processing = True")
    ...
    for i, row in df.iterrows():
        ...
        st.write(f"🔵 Timer state: last_processing_start_after_wash = {last_processing_start_after_wash.strftime('%H:%M') if last_processing_start_after_wash else 'None'}")
        st.write(f"🔵 Reset flag: reset_24h_on_next_processing = {reset_24h_on_next_processing}")
        ...
        while next_scheduled_wash and next_scheduled_wash <= current_time:
            ...
            st.write(f"🟠 SCHEDULED WASH: Adding before product at {next_scheduled_wash.strftime('%H:%M')}")
            tasks.append({
            ...
            reset_24h_on_next_processing = True
            st.write(f"🟠 SCHEDULED WASH: Set reset_24h_on_next_processing = True, new current_time = {current_time.strftime('%H:%M')}")
        ...
        if additional_wash == 'Yes' and wash_duration > timedelta(0):
            ...
            st.write(f"🟢 ADDITIONAL WASH: Adding at {current_time.strftime('%H:%M')}")
            tasks.append({
                'start': current_time,
            ...
            reset_24h_on_next_processing = True
            st.write(f"🟢 ADDITIONAL WASH: Set reset_24h_on_next_processing = True, new current_time = {current_time.strftime('%H:%M')}")

        # Changeover (skip for first product)
        if i > 0:
            st.write(f"🔶 CHANGEOVER: {change_over_mins} minutes starting at {current_time.strftime('%H:%M')}")
            ...
            if next_scheduled_wash and next_scheduled_wash < changeover_end and (next_scheduled_wash + wash_duration) > current_time:
                wash_end = next_scheduled_wash + wash_duration
                st.write(f"🔶 CHANGEOVER: Scheduled wash overlaps, using wash instead")
                tasks.append({
                    'start': next_scheduled_wash,
                    'end': wash_end,
                    'task': 'wash',
                ...
                last_wash_end_time = wash_end
                reset_24h_on_next_processing = True
                st.write(f"🔶 CHANGEOVER: Wash set reset_24h_on_next_processing = True, new current_time = {current_time.strftime('%H:%M')}")
                next_scheduled_wash = wash_end + gap_duration
                ...
                st.write(f"🔶 CHANGEOVER: Overlapping with additional wash {changeover_start.strftime('%H:%M')} to {additional_wash_end.strftime('%H:%M')}")
                tasks.append({
                ...
                # Normal changeover
                st.write(f"🔶 CHANGEOVER: Normal changeover {current_time.strftime('%H:%M')} to {changeover_end.strftime('%H:%M')}")
                tasks.append({
                    'start': current_time,
                    'end': changeover_end,
                    'task': 'changeover',
        ...
        st.write(f"⚙️ PROCESSING: {processing_hours:.2f} hours from {processing_start.strftime('%H:%M')} to {processing_end.strftime('%H:%M')}")
        ...
        if reset_24h_on_next_processing:
            ...
            reset_24h_on_next_processing = False
            st.write(f"🔴 TIMER RESET: Setting timer origin to {last_processing_start_after_wash.strftime('%H:%M')} (processing start after wash)")
            ...
            last_processing_start_after_wash = processing_start
            st.write(f"🔴 TIMER START: Setting initial timer origin to {last_processing_start_after_wash.strftime('%H:%M')} (first product)")

        st.write(f"🔴 CURRENT TIMER: Origin = {last_processing_start_after_wash.strftime('%H:%M') if last_processing_start_after_wash else 'None'}")

        # === Build wash interruptions robustly ===
        # We'll iteratively add scheduled washes that fall within the evolving processing window,
        # then check if the standalone 24h intermediate lands within that window. If adding
        # a standalone forces more scheduled washes to fall inside, the loop repeats until stable.
        ...
        while True:
            ...
            if last_processing_start_after_wash:
                ...
                st.write(f"🔴 24HR CHECK: Timer origin {last_processing_start_after_wash.strftime('%H:%M')} + 24hr = {standalone_due.strftime('%H:%M')}")
                st.write(f"🔴 24HR CHECK: Processing window {processing_start.strftime('%H:%M')} to {window_end.strftime('%H:%M')}")
                
                if processing_start <= standalone_due <= window_end:
                    st.write(f"🔴 24HR CHECK: Standalone due time falls within processing window")
                    ...
                    st.write(f"🔴 24HR CHECK: Has intermediate in period? {has_intermediate_in_period}")
                    
                    if not has_intermediate_in_period:
                        st.write(f"🔴 STANDALONE ADDED: Adding standalone intermediate at {standalone_due.strftime('%H:%M')}")
                        wash_interruptions.append({
                        ...
                        added_any = True
                        # After adding standalone, loop will recheck scheduled washes that may now fall before new window_end
                    else:
                        st.write(f"🔴 STANDALONE SKIPPED: Intermediate already exists in period")
                else:
                    st.write(f"🔴 24HR CHECK: Standalone due time outside processing window")
        ...
        st.write(f"⚙️ PROCESSING END: Product complete at {current_time.strftime('%H:%M')}")
        ...
        if wash_interruptions:
            ...
            st.write(f"🔴 END CHECK: Last wash ended at {last_wash['end'].strftime('%H:%M')}, segment_start = {segment_start.strftime('%H:%M')}")
            ...
            if last_wash['end'] <= segment_start and last_wash.get('type') in ('scheduled', 'standalone_intermediate'):
                st.write(f"🔴 TIMER UPDATE: Updating timer origin to {segment_start.strftime('%H:%M')} (processing resumed after wash)")
                ...
                st.write(f"🔴 TIMER UNCHANGED: Timer origin remains {last_processing_start_after_wash.strftime('%H:%M') if last_processing_start_after_wash else 'None'}")
        else:
            st.write(f"🔴 NO WASHES: Timer origin remains {last_processing_start_after_wash.strftime('%H:%M') if last_processing_start_after_wash else 'None'}")


    # === Visualization (with labels restored) ===
    ...
    for product_name in product_order:
        ...
        for _, task in product_tasks.iterrows():
            ...
            # Time-only labels instead of full datetime
