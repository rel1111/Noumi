...
def generate_timeline(df):
    ...
    for i, row in df.iterrows():
        ...
        # Set processing start after wash if this is the first processing after any wash
        # Only set this for the very first product OR after a wash has occurred
        if last_processing_start_after_wash is None and i == 0:
        ...
        # Reset and restart the 24hr clock ONLY if any wash occurred during this product's processing
        # Changeovers should NOT reset the counter - the timer should continue across changeovers
        had_any_wash = len(wash_interruptions) > 0
        if had_any_wash:
            # A wash occurred, so restart the 24hr timer from when processing resumes
