...
def generate_timeline(df):
    ...
    for i, row in df.iterrows():
        ...
        if additional_wash == 'Yes' and wash_duration > timedelta(0):
            ...
            reset_24h_on_next_processing = True
            # Update next scheduled wash timer - additional wash pushes back the 52hr schedule
            next_scheduled_wash = additional_wash_end + gap_duration
    ...
    ax.set_xlabel("Time")
    # Create title with date from the task data
    if not tasks_df.empty:
        earliest_time = tasks_df['start'].min()
        week_date = earliest_time.strftime('%Y-%m-%d')
        ax.set_title(f"Weekly Production Plan Timeline - Week of {week_date}")
    else:
    ...
    # Add timestamp in bottom right corner with black italic styling
