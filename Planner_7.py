...
def generate_timeline(df):
    ...
    for i, row in df.iterrows():
        ...
        if additional_wash == 'Yes' and wash_duration > timedelta(minutes=0):
            additional_wash_start = current_time
            additional_wash_end = current_time + wash_duration
            tasks.append({
                'start': additional_wash_start,
                'end': additional_wash_end,
                'duration_hours': wash_duration.total_seconds() / 3600,
                'task': 'wash',
                'product': 'Scheduled Wash', # Group with other washes
                'order': i - 0.5 # Order before the current product's tasks but after previous products
            })
            # Simultaneous intermediate wash
            tasks.append(add_intermediate_wash(additional_wash_start, additional_wash_end, wash_duration))
            last_intermediate_wash_time = additional_wash_end
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            optional_columns = ['First Wash Time', 'Intermediate Wash Duration', 'Additional Wash']
