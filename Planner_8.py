    ...
    for i, row in df.iterrows():
        ...
        if i > 0:
            ...
            if (wash_duration > timedelta(0) and 
                ...
                # Allow changeover to happen during wash time
                tasks.append({
                    'start': current_time,
                    'end': changeover_end,
                    'task': 'changeover',
                ...
                # Add the wash that occurs during changeover
                tasks.append({
                ...
                current_time = max(changeover_end, next_wash_time + wash_duration)
                last_wash_end_time = next_wash_time + wash_duration
            else:
                # Normal changeover
                tasks.append({
                    'start': current_time,
        ...
        if last_processing_start:
            ...
            if processing_start <= standalone_due <= processing_end:
                ...
                has_intermediate_in_period = any(
                    w['start'] >= last_processing_start and w['start'] <= standalone_due
        ...
        # Reset the 24hr clock if any wash occurred during this product's processing
        had_any_wash = len(wash_interruptions) > 0
        if had_any_wash:
            last_processing_start = current_time  # Reset 24hr timer after any wash
